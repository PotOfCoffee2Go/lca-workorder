...
<script>
  ...
  document.getElementById("show-records").addEventListener("click", () => {
	...
	function displayForm(type, idx = 0) {
		...
		console.log('idx', idx);
		document.getElementById(`schema-${type}`).innerHTML = 
		...
		if (idx > 0) {
			poc2go.dom[`btn-${type}-prev`].classList.add('btn-enabled');
		}
		else {
			poc2go.dom[`btn-${type}-prev`].classList.remove('btn-enabled');
		}
		if (idx < nbrRecs-1) {
			poc2go.dom[`btn-${type}-next`].classList.add('btn-enabled');
		}
		else {
			poc2go.dom[`btn-${type}-next`].classList.remove('btn-enabled');
		}
	}
	...
	function apiRequest(apireq) {
		...
		.then(data => {
			console.log(data);
			let msg = document.getElementById('msg-' + apireq.api_type);
			...
			if ( nbrRecs === 0) {
					msg.innerHTML = 'No records returned';
			}
			else {
				msg.innerHTML = `Success - ${ns.form[apireq.api_type].length} record${nbrRecs === 1 ? '' : 's'}`;
			}
